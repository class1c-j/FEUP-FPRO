...
def tfidf(documents):
    ...
    for i in documents:
        for j in i.split():
            v.append(j.lower())
    vocab = [b for b in v if b not in vocab]

    wc = {word: len([x for x in documents if word.lower() in x.lower().split()]) for word in vocab}
    print(wc)

    return {word.lower(): [round(i.lower().split().count(word.lower()) * log(len(documents)/wc[word]), 3) for i in documents] for word in vocab}
